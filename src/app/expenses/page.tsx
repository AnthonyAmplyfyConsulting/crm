            ...
            <div className="pt-32 px-6 pb-12 max-w-7xl mx-auto relative z-10">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Manage and track business expenditures.</p>
                    </div>

                    <div className="flex items-center gap-6">
                        <div className="text-right hidden md:block">
                            <p className="text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider">Total Expenses</p>
                            <p className="text-2xl font-bold font-heading text-gray-900 dark:text-white">${totalExpenses.toFixed(2)}</p>
                        </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
                                    ...
                                    <th className="p-4 font-medium text-gray-500 dark:text-gray-400 text-sm">Submitted By</th>
                                    ...
                                    <th className="p-4 font-medium text-gray-500 dark:text-gray-400 text-sm text-center">Receipt</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                                {expenses.map((expense) => (
                                    <tr key={expense.id} className="hover:bg-blood-orange-50/30 dark:hover:bg-blood-orange-950/20 transition-colors">
                                        <td className="p-4 font-medium text-gray-900 dark:text-gray-100">{expense.description}</td>
                                        <td className="p-4 text-gray-600 dark:text-gray-400">{expense.submittedBy}</td>
                                        <td className="p-4 text-gray-600 dark:text-gray-400">{expense.date}</td>
                                        <td className="p-4">
                                            <span className={`px-2 py-1 rounded text-xs font-medium ${expense.status === 'Approved' ? 'text-green-700 bg-green-100 dark:text-green-400 dark:bg-green-900/30' :
                                                expense.status === 'Rejected' ? 'text-red-700 bg-red-100 dark:text-red-400 dark:bg-red-900/30' :
                                                    'text-yellow-700 bg-yellow-100 dark:text-yellow-400 dark:bg-yellow-900/30'
                                                }`}>
                                                {expense.status}
                                            </span>
                                        </td>
                                        <td className="p-4 text-gray-900 dark:text-gray-100 font-medium text-right">${expense.amount.toFixed(2)}</td>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl w-full max-w-md p-6 relative shadow-2xl">
                        ...
                        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-6 font-heading">Add New Expense</h2>
                        ...
                        <form onSubmit={handleAddExpense} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Receipt Image</label>
